...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'h4o77Bjn&ml:od***h$'
...
def post(post_id):
    ...
    result = cursor.execute(
        'SELECT * from comment JOIN user on comment.user_id = user.id WHERE comment.post_id = ?', (post_id,)
    ).fetchall()
    comments = []
...
def user_commenting(user_id, post_id):
...
@app.route('/Phasmophobia/')
def phasmophobia():
    return render_template('Phasmophobia.html')
